    ...
    if (onlyExists) {
      if (!getScrollState()) {
        $target.style.overflowY =
          $target.dataset.scrollLockSavedOverflowYProperty;
      }
    } else {
...
const hideLockableOverflowTarget = $target => {
  if (isElement($target) && $target.dataset.scrollLockLocked !== 'true') {
    ...
    $target.dataset.scrollLockSavedInlineOverflowProperty =
      $target.style.overflow;
    $target.dataset.scrollLockSavedInlineOverflowYProperty =
      $target.style.overflowY;
...
const unfillGapTarget = $target => {
  if (isElement($target)) {
    if ($target.dataset.scrollLockFilledGap === 'true') {
      const currentFillGapMethod =
        $target.dataset.scrollLockCurrentFillGapMethod;
